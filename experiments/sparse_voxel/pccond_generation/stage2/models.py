...
from experiments.sparse_voxel.pccond_generation.stage1.models import PointTransformerEncoder, TimestepEmbedder, timestep_embedding
...
class SparseVertexSDFTransformer(nn.Module):
    ...
    def __init__(
        ...
        # 3D Vertex Coordinate Fourier Embedder
    ...
    def forward(
        ...
        **kwargs
        ...
        :param cond: [T_total, 3 + cond_dim] unified condition tensor (or coords / pc)
        ...
        if cond is not None and cond.shape[-1] >= 3 + self.cond_dim:
            if cond.ndim == 3:
                cond = cond.squeeze(0)
            coords = cond[:, :3]
            c_pc_tokens = cond[:, 3:3+self.cond_dim]
        ...
        if c_pc_tokens is not None:
            c_pc_proj = self.cond_proj(c_pc_tokens) # [T_total, embed_dim]
            c = t_emb_stacked + c_pc_proj
        elif pc is not None:
            c_pc = self.pc_encoder(pc)
            c_pc_proj = self.cond_proj(c_pc)
            if batch_size > 1 and len(c_pc_proj) == batch_size:
                c_pc_stacked = torch.repeat_interleave(c_pc_proj, seq_lens_tensor, dim=0)
            else:
                c_pc_stacked = c_pc_proj.expand(s.shape[0], -1)
            c = t_emb_stacked + c_pc_stacked
        else:
            c_pc = self.pc_encoder(None, batch_size=batch_size)
            c_pc_proj = self.cond_proj(c_pc)
            c_pc_stacked = torch.repeat_interleave(c_pc_proj, seq_lens_tensor, dim=0)
            c = t_emb_stacked + c_pc_stacked
