...
def main():
    parser = argparse.ArgumentParser(description="Train Stage 2 Stacked 1D Sparse Vertex SDF Rectified Flow on Digit3D")
    ...
    print("==================================================================")
    print(" Digit3D Stage 2 Native VarLen Sparse Vertex SDF Rectified Flow   ")
    print("==================================================================")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step_idx, (bmesh, imgs, _) in enumerate(pbar):
            bmesh = bmesh.to(device)
            imgs = imgs.to(device)

            with torch.no_grad():
                sparse_coords, sparse_sdfs = mesh2sparse(bmesh, res=[32, 32, 32], grid_bound=1.2, iso=0.0)
                stacked_verts, stacked_sdfs, seq_lens_t, seq_lens_list = prepare_stacked_vertex_batch(
                    sparse_coords, sparse_sdfs, bmesh.batch_size, device
                )

            # Broadcast 2D image condition across each sample's active vertices
            c_img = model.img_encoder(imgs) # [B, embed_dim]
            c_img_stacked = torch.repeat_interleave(c_img, seq_lens_t, dim=0).unsqueeze(0) # [1, T_total, embed_dim]
            cond_tensor = torch.cat([stacked_verts, c_img_stacked], dim=-1) # [1, T_total, 3 + embed_dim]

            model.set_seq_lens(seq_lens_list)
            loss = rf(stacked_sdfs, cond=cond_tensor) / args.accum_steps
            loss.backward()
        ...
        with torch.no_grad():
            for bmesh, imgs, _ in test_loader:
                ...
                c_img = model.img_encoder(imgs)
                c_img_stacked = torch.repeat_interleave(c_img, seq_lens_t, dim=0).unsqueeze(0)
                cond_tensor = torch.cat([stacked_verts, c_img_stacked], dim=-1)
    ...
    print(f"\nStage 2 Training Complete! Best Val Loss: {best_val_loss:.4f}")
